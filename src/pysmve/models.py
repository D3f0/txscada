...
database = PostgresqlDatabase('smve', user='postgres', host='localhost', port=5432)
...
def insert_comaster(profile, data):
    ...
    for offset, canvarsys, candis, canais, addr_485_IED in config:
        ...
        if addr_485_IED == 1:
            ...
            VarSys(ied=ied,
                   param="Sesgo",
                   description="Sesgo (entero)",
                   unit="ms").save()
            VarSys(ied=ied,
                   param="Calificador",
                   description="Calif Low/Errores High",
                   unit="Ciclos").save()
            # Crear DIs
            ...
            AI(ied=ied,
               param="P",
               description=u"Potencia Activa",
               unit="Kw",
               multip_asm=DEFAULT_ASM_MULTIP,
               divider=1,
               relacion_tv=1,
               relacion_ti=1,
               relacion_33_13=2.5).save()

            AI(ied=ied,
               param="Q",
               description=u"Potencia Reactiva",
               unit="Kvar",
               multip_asm=DEFAULT_ASM_MULTIP,
               divider=1,
               relacion_tv=1,
